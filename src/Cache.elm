module Cache exposing (Cache, empty, insert, safeEmpty, errorToString, member, get, CacheError(..))

import PositiveInt exposing (PositiveInt)

type Cache tag value = Cache (CacheStructure tag value)

type CacheError = ItemTooBig { cacheMax: PositiveInt, itemSize: PositiveInt }
...
type alias CacheItem tag value = { size: PositiveInt, tag: tag, value: value }
...
ejectUntilSize (Cache cache) target_size =
    ...
    let
        (Cache emptyCache) = safeEmpty cache.sizeMax
        ...
        takeWhile acc items =
            case items of
                ...
                item :: remaining ->
                    let
                        size_afterwards = PositiveInt.add acc.sizeCurrent item.size
                    in
                    if (PositiveInt.unwrap size_afterwards) <= (PositiveInt.unwrap target_size) then
                        ...
                        { acc | items = List.reverse acc.items }
    in Cache (takeWhile emptyCache cache.items)
...
insert (Cache cache) item =
        case PositiveInt.subs cache.sizeMax item.size of
            Ok targetSize ->
                let
                    (Cache withSpace) = ejectUntilSize (Cache cache) targetSize
                in
                Ok <| Cache { withSpace |
                        items = item :: withSpace.items,
                        sizeCurrent = PositiveInt.add cache.sizeCurrent item.size
                }
            Err _ -> Err <| ItemTooBig { cacheMax = cache.sizeMax, itemSize = item.size }
...
get (Cache cache) tag =
        ...
        let
            getHelp list = case list of
                    [] -> Nothing
                    x::xs -> if x.tag == tag then Just x.value else getHelp xs
        in getHelp cache.items
...
member (Cache cache) tag =
        ...
        -- work, search the list from most recent to oldest should have good performance...
        let
            memberHelp list = case list of
                    [] -> False
                    x::xs -> if x.tag == tag then True else memberHelp xs
        in memberHelp cache.items
